...
/// Represents an arc.
/// Holds every mathematically calculated value that can be used later.
#[derive(Debug, Clone, Copy)]
pub struct Arc {
    pub start: Point,
    pub end: Point,
    ...
    pub fn build(
        ...
        match method {
            CircleMethod::RelativePoint(ref rel_center) => {
                ...
                let start_planar = start.planar(plane);
                let end_planar = end.planar(plane);
...
impl Machine {
    ...
    pub fn move_machine_pos(
        ...
        Ok(MotionSummary::Rapid(Line::new(org_pos, new_pos)))
    }
            ...
            Motion::Feed => {
                match self.feed {
                    Some(_) => {
                        ...
                        let new_pos = self.new_pos(pos);
                        self.set_pos(new_pos)?;
                        Ok(MotionSummary::Feed(Line::new(org_pos, new_pos)))
                    }
...
///
/// On success, returns a *tuple* consisting of three fields:
/// - The *center [`PlanarPoint`]* of the arc that satisfies the given conditions.
/// - The *radius* of the said arc
/// - The [`ArcType`] of the said arc.
///
...
fn arc_info(
    ...
    method: CircleMethod,
    dir: CircularDirection,
) -> Result<(PlanarPoint, Float, ArcType), MachineError> {
    ...
    match method {
        CircleMethod::RelativePoint(ref rel_center) => {
            ...
            // direction towards start pos from center
            let to_start = *current_pos - center;
            // direction towards end pos from center
            let to_end = *new_pos - center;
        ...
        CircleMethod::FixedRadius(radius) => {
            ...
            if (2.0 * radius - dist).abs() < 1e-10 {
                return Ok((midpoint, radius, arc_type));
            }
            ...
            // TODO create diagram to prove the result for all 4 quadrants
            let center = match (arc_type, dir) {
                (ArcType::Minor, CircularDirection::Clockwise)
                | (ArcType::Major, CircularDirection::CounterClockwise) => PlanarPoint::new(
                    plane,
                    midpoint.first() + (bisector * delta.second()) / dist,
                    midpoint.second() - (bisector * delta.first()) / dist,
                ),
...
mod tests {
    ...
    fn test_arc_info() {
        ...
        let (center, radius, arc_type) = arc_info(
            &start,
            &end,
            method.clone(),
            CircularDirection::CounterClockwise,
